    ...
    <tbody className="border border-red-600">
        {attendances.map(attendance => (
          <tr key={attendance.attendanceId} className="h-10 border-transparent border-2">
            ...
            <td className="">
                <img src={scheduleImg} className="w-8 m-auto"/>
            </td>
        </tr>
        ))}
    </tbody>
...
function ListAttendances() {
  ...
  useEffect(()=>{
    const fetchData = async ()=>{
      ...
      const attendancesData = await attendanceService.getAll({attendanceDate: selectedDate, page: 1, perPage: 10})
    ...
    <div className="w-full">
      <div className="max-w-5xl m-auto pt-10">
        ...
        <div className="shadow-md bg-[#D9D9D970] py-3">
            <AttendanceTable attendances={lastAttendances}></AttendanceTable>
        </div>
